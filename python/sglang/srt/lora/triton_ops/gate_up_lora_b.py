...
# BLOCK_N tiles output_dim (per-slice output width); BLOCK_K tiles rank.
DEFAULT_GATE_UP_B_CONFIG = {"BLOCK_S": 16, "BLOCK_N": 64, "BLOCK_K": 16}
...
def gate_up_lora_b_fwd(
    ...
    # output_dim (per-slice output width); BLOCK_K tiles K = rank.
    cfg = get_sgemm_b_config(
        K=output_dim,
        R=r,
        num_slices=2,
        max_len=batch_info.max_len,
        default=DEFAULT_GATE_UP_B_CONFIG,
    )
    BLOCK_S = cfg["BLOCK_S"]
